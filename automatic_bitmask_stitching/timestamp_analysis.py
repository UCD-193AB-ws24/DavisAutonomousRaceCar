...
def plot_distributions(image_diffs, pose_diffs):
    ...
    plt.hist(pose_diffs, bins=50, alpha=0.5, label='Poses')
    ...
    plt.hist(pose_diffs, bins=50, alpha=0.5, label='Poses', cumulative=True, density=True)
    ...
    # Plot 3: Box Plot
    plt.subplot(2, 2, 3)
    plt.boxplot([image_diffs, pose_diffs], labels=['Images', 'Poses'])
    ...
    plt.scatter(range(len(pose_diffs)), pose_diffs, alpha=0.5, label='Poses', s=10)
    ...
    plt.tight_layout()
    plt.savefig('data/datarun_for_stitching_1/output/visualizations/timestamp_analysis.png', dpi=300, bbox_inches='tight')
    print("\nVisualization saved as 'data/datarun_for_stitching_1/output/visualizations/timestamp_analysis.png'")

def main():
    # Load timestamps
    image_timestamps = load_timestamps('data/datarun_for_stitching_1/output/image_timestamps.csv', 'image_timestamp_sec')
    pose_timestamps = load_timestamps('data/datarun_for_stitching_1/output/pose_timestamps.csv', 'pose_timestamp_sec')
    
    # Analyze timestamps
    image_diffs, image_stats = analyze_timestamps(image_timestamps, 'Image')
    pose_diffs, pose_stats = analyze_timestamps(pose_timestamps, 'Pose')
    ...
    pose_coverage = (overlap_duration / (pose_timestamps[-1] - pose_timestamps[0])) * 100
    
    print(f"\nCoverage Analysis:")
    print(f"Image Data Coverage: {image_coverage:.1f}%")
    print(f"Pose Data Coverage: {pose_coverage:.1f}%")
    
    # Create visualizations
    plot_distributions(image_diffs, pose_diffs)
